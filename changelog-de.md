...
- City Builder:
    - Neuer „Umbau-Assistent“: Er übernimmt das gewählte Layout und führt durch den Umbaumodus des Spiels. Die Stadt wird von hinten nach vorn gezeichnet, ein soeben gesetztes Gebäude verdeckt also den Boden dahinter — deshalb sortiert der Assistent den ganzen Plan in Streifen quer über den Bildschirm und arbeitet sie von hinten ab, Streifen für Streifen, damit der Platz für das nächste Gebäude nie vom letzten verdeckt wird. Jeder Platz wird abgehakt, sobald das Spiel die Platzierung meldet — ein Platz ist eine Grundfläche mit Straße daneben, kein bestimmtes Gebäude, also zählt jedes Gebäude derselben Größe, solange es nicht mehr Straße braucht als der Platz vorsieht; der Tausch wird vermerkt, denn das ursprünglich vorgesehene Gebäude braucht nun einen der anderen Plätze. Was noch im Weg steht wird beim Namen genannt, ein Gebäude an einer Stelle, die der Plan nicht vorsieht, wird gemeldet, der nächste geplante Platz jedes Gebäudes steht in der Größenliste des Umbaumenüs, und der Plan lässt sich mit markiertem Streifen über die Stadt legen. Der Plan wird im Browser gespeichert und die Speicherung durch Rücklesen geprüft, ein Neuladen kostet also nicht die Suche von vorn — er trägt sein Datum, kommt beim erneuten Betreten des Umbaumodus mit seinem Fortschritt zurück und lässt sich jederzeit aus der Stadtkarte wieder öffnen. Ein Layout für eine nur besuchte Stadt wird nie gespeichert: Es würde den einen Plan überschreiben, den der Assistent hält — den eigenen
    - Das günstigste vollständige Layout der Suche steht jetzt immer unter den Varianten, mit „wenigste Straßen“ beschriftet, und nichts sortiert es aus. Den freien Platz nach seiner Nutzbarkeit zu bewerten hatte einen Nebeneffekt: Ein Plan, der zwei Kacheln weniger kostet, seinen freien Platz aber verstreut, wurde zugunsten des geräumigeren verworfen — falsch, wenn Straßenkacheln das Einzige sind, was zählt
    - Behoben: Beim Wechsel auf eine andere Variante las der Bericht die Kennzahlen des Laufs vom angezeigten Layout ab und behauptete „0 Varianten in 0 s“; die Zeile, was die Suche aus deiner bestehenden Stadt gemacht hat, fiel dabei stillschweigend weg
    - Die Stadt, die du schon hast, wird nicht mehr nur gemessen, sondern auch nachgebessert: Die beiden Durchgänge, die ein rohes Layout von 280 auf 169 Straßenkacheln bringen — erst jede Kachel zurücknehmen, die niemand braucht, dann die Gebäude heranziehen, deren Stichstraßen der Grund für den Rest sind — laufen jetzt auch über dein eigenes Layout. Fällt das günstiger aus, steht es als eigene Variante zur Wahl, und es ist der einzige Plan, den du wirklich umsetzen kannst, weil fast jedes Gebäude stehen bleibt
    - Behoben: Die Größe des reservierten freien Quadrats richtete sich nach dem Layout, das in der Wertung gerade vorn lag, statt nach der Größe, die die Suche soeben als möglich nachgewiesen hatte. Straßenkacheln zählen zuerst, also führte regelmäßig ein Layout mit 3×3-Quadrat, während 8×8 nachweislich passte — und jeder spätere Lauf erbte das kleine. Heraus kamen Pläne, deren freier Platz ein Feld einzelner Kacheln war: 61 von 74 freien Kacheln lose bei einer Stadt, gegenüber 10 mit richtig reserviertem Quadrat, bei einer Straßenkachel Unterschied. Jede Rathaus-Position bekommt jetzt zusätzlich einen gezielten Lauf mit dem Quadrat und probiert mehrere Stellen dafür, denn ein an der falschen Seite ausgeschnittenes Quadrat kann das Layout Gebäude kosten
    - Die Suche entscheidet den Handel zwischen Straßen und Platz nicht mehr für dich: Sie behält jedes vollständige Layout, das von keinem anderen zugleich bei Straßenkacheln und freiem Quadrat geschlagen wird, und über der Karte wählst du aus. Bei einer Stadt mit 119 Straßenkacheln verwarf die Wertung bisher stillschweigend einen Plan, der zehn Kacheln mehr kostete und dafür ein freies Quadrat von 8×8 statt 6×6 brachte — 64 freie Kacheln am Stück gegen 36. Dein eigenes Layout steht mit zur Wahl, wann immer es gewinnt, und der HTML-Export trägt alle Varianten in einer Datei hinter Reitern
    - Das Rathaus landet nicht mehr immer in der oberen linken Ecke. Alle vier Ecken und die Mitte werden jetzt durchsucht — das Distrikt-Layout wird dazu auf der Karte gespiegelt, es ist also dasselbe Problem von der anderen Seite — und jede Position wird als eigene Variante angeboten, benannt danach, wo das Rathaus steht. Welche Ecke die beste ist, hängt vom Zuschnitt der freigeschalteten Fläche ab und ist von Stadt zu Stadt verschieden: bei einer Stadt 7 Straßenkacheln wert, bei einer anderen 4, bei einer dritten nichts. Ein Rathaus in der Mitte kostet auf den meisten Karten ein paar Kacheln, ist aber die einzige Anordnung, in die eine randvolle Stadt hineinpasst — deshalb bleibt sie zur Wahl, auch wenn sie nach Zahlen verliert
    - Neue Schaltfläche „Eingabe speichern“ neben den Bericht-Schaltflächen: Sie schreibt genau das, was der Optimierer bekommt — freigeschaltete Gebiete, Gebäude mit Größen und Straßenbedarf und die Straßenkacheln der Stadt, wie sie steht — in eine nach der Stadt benannte JSON-Datei. Der Diagnosebericht enthielt die Straßenkacheln nie, ein daraus nachgestellter Lauf hatte also nichts, woran er seine Ersparnis messen konnte
    - Das kopierte Log beginnt jetzt mit einer Zusammenfassung in der eigenen Sprache: Straßen vorher und nachher samt Differenz, Ersparnis, platzierte Gebäude, freie Kacheln, freies Quadrat und größtes freies Rechteck, verstreute Kacheln, geprüfte Varianten und Suchdauer. Die technische Spur darunter bleibt englisch, damit ein Fehlerbericht für alle gleich liest
    - Polnische Übersetzung des City Builders vervollständigt — sie hatte 3 von 49 Texten
    - Der freie Platz wird jetzt danach beurteilt, ob sich darauf überhaupt etwas bauen lässt. Straßenkacheln entscheiden weiterhin zuerst, aber nicht mehr auf eine Kachel genau: Layouts, die höchstens fünf Kacheln über dem günstigsten liegen, gelten als gleich teuer, und darunter entscheidet das größte freie Quadrat. 70 freie Kacheln als 40×1-Band verlieren gegen 64 Kacheln als 8×8-Block. Die Toleranz misst gegen das günstigste vollständige Layout, damit eine Kette von "nur drei Kacheln mehr" den Plan nicht Schritt für Schritt davon wegträgt
    - Die Straßenbilanz liegt im Draufsicht-Modus nicht mehr über der Karte. Sie saß in der linken oberen Ecke — genau dort, wo die Stadt gezeichnet wird — und verdeckte die Gebäude, um die es geht; jetzt steht sie als Leiste über der Karte
    - Die bereits gebaute Stadt geht jetzt selbst als Kandidat in die Suche. Sie ist per Definition ein vollständiges, funktionierendes Layout, also kann nichts mehr vorgeschlagen werden, was mehr Straßenkacheln kostet als sie — bisher konnte eine volle Stadt einen "Plan" bekommen, der 25 Straßenkacheln mehr verbrauchte als der vorhandene und obendrein drei Gebäude weglegte. Schlägt keine Variante sie, lautet die Antwort schlicht: dein eigenes Layout gewinnt
    - Neuer Packer für die straßenlosen Gebäude: statt Reihe für Reihe aus einer Ecke zu füllen, setzt er jedes Gebäude in das knappste freie Rechteck der Karte und vermeidet Streifen von genau einer Kachel Breite, in die nichts mehr passt. Ein nur eine Kachel breites Gebäude wird jetzt außerdem zusammen mit den großen bedient — es braucht seine Kacheln in einer Linie und nur in einer Linie, wer ihm die Reste überlässt, lässt es obdachlos. In einer zu 100% vollen Stadt sank die Zahl der Gebäude ohne Platz von 9 auf 2
    - Ein Layout, das Gebäude übrig lässt, bekommt jetzt eine zweite und dritte Chance, statt verworfen zu werden: die gesamte straßenlose Masse wird mit anderen Baureihenfolgen und beiden Schwerkraftrichtungen neu gelegt, und reicht auch das nicht, darf ein übrig gebliebenes Gebäude einen Platz von straßenlosen Nachbarn übernehmen, die sich einen neuen suchen — diese dürfen ihrerseits verdrängen, aber nur Gebäude, die leichter zu setzen sind als sie selbst, sodass die Kette immer bei einem Ein-Kachel-Gebäude endet statt im Kreis zu laufen. Der ganze Tausch wird zurückgenommen, wenn auch nur einer keinen neuen Platz findet
    - Varianten werden jetzt zuerst nach Straßenkacheln und erst danach nach dem freien Quadrat bewertet, wie gewünscht — jede nicht verbaute Kachel bleibt bebaubar, und in einer vollen Stadt kommt der freie Platz genau daher. Das Zeitbudget folgt: die Quadratsuche bekommt ein Drittel der Uhr statt zwei Dritteln, und jede dritte Feinschliff-Runde läuft ohne reserviertes Quadrat, weil ein erzwungenes Quadrat Straßenkacheln kosten kann
    - Das Log nennt jetzt auch den Packer jeder Variante, nicht nur Strategie und Sortierung
    - Behoben: Wohn-, Produktions- und Warengebäude bekamen zwangsweise einen Straßenbedarf, auch wenn die Spieldaten keinen ausweisen — in einer Stadt aus modernen straßenlosen Gebäuden bedeutete das hunderte unnötige Anschlüsse und deutlich mehr Straßenkacheln, als die Stadt überhaupt hatte; der Straßenbedarf wird jetzt genau so gelesen wie im Modul Stadtkarte
    - Der HTML-Export und die Karte zeigen jetzt den freien Platz selbst: jede freigeschaltete Kachel ohne Gebäude wird grün gezeichnet, der freie Block ist damit auf einen Blick sichtbar statt aus den Zahlen erraten. Den Karten im Export fehlte zudem eine explizite Höhe, sie fielen im Browser auf einen Streifen zusammen
    - Jeder Bericht nennt jetzt, wessen Stadt es ist: Spielername und Welt im Log, im JSON, im Titel und Kopfbalken der HTML-Seite und im Dateinamen — zwei Berichte nebeneinander waren zuvor nicht unterscheidbar
    - Der kopierte Bericht ist jetzt kompakt: identische Gebäude werden zu Stückzahlen zusammengefasst, das kachelweise Layout bleibt der gespeicherten Datei vorbehalten — ein eingefügter Bericht schrumpfte von rund 32 000 auf 1 600 Zeichen
    - Ein Knopf statt zwei: „Bericht kopieren" legt jetzt Protokoll und vollständige Daten gemeinsam in die Zwischenablage, die gespeicherte Datei enthält ebenfalls beides
    - Behoben: Die Entitäten anderer Spieler halten ihre Position in einem `coords`-Objekt, weshalb die Seite „Aktuelle Stadt" im Export ein leeres grünes Feld blieb. Lassen sich Koordinaten gar nicht lesen, druckt das Log jetzt die rohen Feldnamen der Entität, statt es dem Raten zu überlassen
    - Die Entitäten anderer Spieler kommen roh aus dem Spiel, daher wird ein verschachteltes Positionsobjekt als Rückfall gelesen. Fehlende Koordinaten lassen ein Gebäude nie herausfallen — sie werden für das Vorher-Bild gebraucht, nie für die Planung selbst
    - Behoben: In der Stadt eines anderen Spielers fiel jedes Gebäude heraus und der Lauf startete auf einer leeren Karte — dessen Instanzen tragen entityId statt cityentity_id, denselben Fallback nutzt das Modul Stadtkarte bereits. Eine leere Gebäudeliste meldet das jetzt, statt ein Layout für nichts zu suchen
    - Die Draufsicht ist wieder ein richtiges Fenster: Holzgrund, Rahmen, Kopfbalken über die volle Breite und eigenes Scrollen. Der flache Plan schwebte zuvor direkt auf der laufenden Stadt und war am Boxrand abgeschnitten, was wie ein Fehler aussah — die isometrische Überlagerung bleibt wie bisher klickdurchlässig
    - Behoben: Die Variantenliste hing nur an erfolgreichen Läufen, ein gescheiterter Lauf — der Fall, in dem sie zählt — zeigte weiterhin nur die Siegerin
    - Das Log listet jetzt die besten Varianten eines Laufs auf, nicht nur die Siegerin: Strategie, Straßen, fehlende Gebäude und Reserve nebeneinander — ein gescheiterter Lauf sagte bisher nichts darüber, warum die anderen Strategien verloren
    - Jeder Fehlschlag endet jetzt im selben Panel mit den Knöpfen „Log kopieren", „Bericht kopieren" und „Bericht speichern" — eine nackte rote Zeile ließ keine Möglichkeit, etwas weiterzugeben, genau dann, wenn das Log am nötigsten ist; das Log benennt den Fehler, und auch ein Lauf, der an den Kartendaten scheitert, ist meldbar
    - Lade- und Meldezustände füllen die Box wieder mit dem Fensterhintergrund des Helpers — die Kartenansicht bleibt eine durchsichtige Überlagerung der Stadt, eine Meldung muss aber für sich lesbar sein
    - Der HTML-Export trägt jetzt das Aussehen des Helpers — Fensterbalken, Holzpanel und die beige/goldene Palette der Box-Styles — und wer ein Gebäude überfährt, sieht dasselbe Gebäude auch auf der anderen Karte markiert
    - Neuer Knopf „HTML-Export": eine eigenständige Seite mit der aktuellen Stadt und dem Plan nebeneinander, darunter die Zahlen — nichts wird aus dem Netz geladen, die Datei kann einfach weitergegeben werden
    - Der City Builder lässt sich jetzt auch für die Stadt eines anderen Spielers öffnen, sofern das Spiel dessen freigeschaltete Gebiete übermittelt hat
    - Die Spalten-Variante belegt keinen Platz mehr in der Basisrunde: jede zusätzliche Basisvariante ist eine Sondierung weniger für die Quadratsuche, und genau die macht aus dem freien Platz einen Block — sie kostete eine Teststadt ihr freies 15x15-Quadrat, das jetzt wieder da ist
    - Das Viertel baut jetzt auch Straßenspalten: ein Gebäude, das breiter als hoch ist, kostet an einer senkrechten Linie weniger Front als an einer waagerechten (ein 6x4 braucht 4 statt 6 Kacheln), deshalb werden solche Gebäude aus Spalten versorgt, die an der letzten Reihenlinie hängen. Beide Varianten — mit und ohne Spalten — werden probiert, denn welche gewinnt, hängt von der Form der Karte ab (in einer Teststadt der gemeldeten Dichte: 136 -> 126 Straßenkacheln)
    - Das Statistik-Panel und das Log nennen jetzt auch das größte freie Rechteck — der nutzbare freie Platz ist oft breiter als das Quadrat, das hineinpasst
    - Der freie Platz wird jetzt danach beurteilt, wie viel davon an einem Stück liegt; die quadratische Form ist Vorliebe statt Bedingung — in zwei Teststädten brachte das deutlich weniger Straßen und viel weniger Streuung statt eines großen Quadrats (137 -> 123 und 43 -> 37 Straßenkacheln, verstreute Kacheln etwa halbiert)
    - Neuer Regal-Packmodus für die straßenlose Masse, der neben dem bisherigen probiert wird: Gebäude werden nach Höhe gruppiert und jedes Regal nimmt nur gleich hohe Stücke auf, damit der Restplatz rechteckig statt ausgefranst bleibt. Er gewinnt nur dort, wo er wirklich besser ist
    - Gebäude ohne Platz werden erneut versucht, sobald die Nachverdichtung die Fugen geschlossen hat — die Nischen, die sie brauchen, entstehen oft erst dann. Die Ablaufspur meldet, wie viele der zweite Versuch gerettet hat
    - Neuer schneller Packer für Gebäude ohne Straßenbedarf: der alte durchsuchte für jedes einzelne Gebäude alle freien Regionen der ganzen Karte, was Sekunden pro Lauf kostete und trotzdem überall Fugen hinterließ. Der neue packt sie dicht aus einer Ecke — in einer Teststadt mit 428 Gebäuden sank ein Lauf von 9,7 s auf 1,1 s und der freie Platz von 136 verstreuten Kacheln auf 26
    - Das Zeitbudget der Suche liegt wieder bei 90 Sekunden: da die Läufe eine Größenordnung schneller sind, findet sie in 90 s mehr als der alte Code in 10 Minuten — in der Teststadt wuchs das freie Quadrat von 3×3 auf 15×15
    - Das Viertel legt sein erstes Band als doppelte Straßenlinie an und versorgt die zweispurigen Gebäude daraus — ein einziges zweispuriges Gebäude war zuvor das, was das gesamte Layout fallen lassen musste
    - Neue Layout-Strategie „Viertel" für Städte, in denen nur wenige Gebäude eine Straße brauchen: die Band-Strategien pflastern die ganze Karte mit Straßenreihen und schneiden hinterher zurück — in einer Stadt mit 428 Gebäuden, davon 56 mit Straßenbedarf, beginnt das bei 367 Straßenkacheln, während die Karte nur Platz für 196 hat. Das Viertel legt stattdessen eine gerade Linie je Band, nur so lang wie die Fronten es verlangen, mit Gebäuden auf beiden Seiten, die ihre kurze Seite zur Straße zeigen, und einer einzigen geraden Achse, die alle Linien verbindet
    - Neuer Straßen-Verkürzungslauf: das Zurückschneiden allein entfernt nur Kacheln, die niemand braucht — der meiste Straßenüberschuss existiert aber nur, um ein zu weit entferntes Gebäude zu erreichen; solche Ausreißer werden jetzt an eine bestehende Straße herangezogen, sodass ihr ganzer Stichweg verschwindet (in einer dichten Teststadt mit 430 Gebäuden: 70 → 37 Straßenkacheln)
    - Behoben: In den Lade- und Meldepanels ließ sich kein Knopf anklicken — das City-Builder-Fenster ist eine klickdurchlässige Überlagerung der laufenden Stadt, und die neuen Panels holten sich die Mausereignisse nicht zurück; das tun sie jetzt und folgen dem Aussehen der bestehenden Steuerleiste, statt ein eigenes Fenster zu zeichnen
    - Behoben: Das Kopieren von Bericht und Log blieb wirkungslos — der Zwischenablage-Helfer steigt aus, solange die Spiel-Leinwand den Fokus hält; beide Knöpfe nutzen jetzt wie die übrigen Module den Legacy-Weg und bestätigen mit einem Toast
    - Melde- und Ladezustände füllen die Box jetzt mit undurchsichtigem Hintergrund, statt die laufende Stadt durchscheinen zu lassen
    - Wird kein vollständiges Layout gefunden, lässt sich der beste Versuch mit „Layout trotzdem anzeigen" ansehen — als Vorschlag gilt er nicht, aber bei einem einzigen fehlenden Kleingebäude lohnt der Blick
    - Neuer Knopf „Log kopieren": ein kurzes lesbares Protokoll des Laufs — Straßenbudget der Karte, Straßenbilanz, was jede Phase der Siegervariante bewirkt hat, und die übrig gebliebenen Gebäude
    - Ein Layout, das Gebäude weglässt, wird gar nicht mehr vorgeschlagen: Vollständigkeit ist jetzt harte Bedingung statt nur erstes Ranking-Kriterium — wird kein vollständiges Layout gefunden, meldet der Lauf das Scheitern und listet die Gebäude ohne Platz auf
    - Die Suche misst jetzt, wie lange ein einzelner Lauf in der echten Stadt dauert, und teilt das Budget entsprechend auf — in einer großen Stadt fraß eine einzige Sondierung zuvor Minuten des Budgets
    - Neue Knöpfe „Bericht kopieren" und „Bericht speichern": Karte, Gebäudeliste mit Größen und Straßenbedarf, das berechnete Layout und alle Statistiken als JSON — um ein schlechtes Layout zu melden, ohne in der Konsole zu graben
    - Neuer Schalter „Draufsicht" (standardmäßig an): der Plan wird flach auf undurchsichtigem Hintergrund gezeichnet statt schräg über die laufende Stadt gelegt, wo er mit der Karte verschwamm und kaum zu beurteilen war
    - Die Layout-Suche verfolgt jetzt zwei klare Ziele in dieser Reihenfolge: das größtmögliche freie Quadrat, das am Stück frei bleibt, und danach möglichst wenige Straßenkacheln — das Quadrat wird vorab reserviert und die ganze Stadt darum herum gebaut, sodass der freie Platz ein nutzbarer Block ist statt verstreuter Restfugen
    - Die Ladeanzeige zeigt die aktuelle Phase, die Restzeit, die Zahl der geprüften Varianten und das bisher beste Ergebnis — ein Knopf bricht die Suche vorzeitig ab und übernimmt dieses beste Layout
    - Das Statistik-Panel nennt zusätzlich die Kantenlänge des freien Quadrats und wie viele freie Kacheln außerhalb davon liegen
